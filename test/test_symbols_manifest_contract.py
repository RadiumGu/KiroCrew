...
def test_the_release_gate_cannot_red_a_stable_release() -> None:
    """The fail-closed check must not fire on the channel that builds nothing.

    ``build-desktop`` is ``if: channel == 'insider'``, so a stable run produces no
    manifest of its own -- it promotes an already-released insider build. An
    unconditional ``manifests > 0`` therefore fails the ``github-release`` job on
    every stable tag, publishing no Release page and no assets at all: the exact
    v0.3.0 incident release.yml's own comment records. Pinned because that failure
    only appears on a real stable tag, which no PR run exercises.
    """
    doc = yaml.safe_load(RELEASE_WORKFLOW.read_text(encoding="utf-8"))
    jobs = doc["jobs"]
    assert jobs["build-desktop"]["if"].strip() == "needs.version.outputs.channel == 'insider'", (
        "build-desktop's channel condition changed; re-derive which channels can "
    ...
    assert 'if [ "$CHANNEL" = "insider" ] && [ "$manifests" -eq 0 ]; then' in script, (
        "the symbols-manifest requirement is not scoped to the insider channel, "
        "so a stable release run finds zero manifests and fails the job that "
        "publishes the GitHub Release"
    )
    assert (
        steps[assemble].get("env", {}).get("CHANNEL") == "${{ needs.version.outputs.channel }}"
    ), (
