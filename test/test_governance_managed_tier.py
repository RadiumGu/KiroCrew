...
#: The absence guard itself, and the dotted path the tests patch. The guard no longer
#: consults ``path_writable_by_current_user`` -- it reads the owner and mode of the
#: directory that would HOLD the document -- so this is the seam that matters now.
#: Captured at import, BEFORE the autouse fixture replaces the module attribute.
ABSENCE_PRED_PATH = "kiro_crew.platform.governance._absence_could_be_self_inflicted"
_REAL_ABSENCE_PRED = governance._absence_could_be_self_inflicted


def _fake_dir_stat(*, uid: int, mode: int) -> os.stat_result:
    """A crafted ``stat_result`` for a directory, for cases a test cannot create.

    A test cannot chown a directory to root, so the root-owned branches are exercised
    against a synthetic stat. The sticky and non-root branches ARE creatable and are
    tested for real; this covers only what the filesystem will not let a test build.
    """
    return os.stat_result((stat.S_IFDIR | mode, 0, 0, 1, uid, 0, 4096, 0, 0, 0))
...
#: The absence guard only REFUSES where the writability predicate can actually answer.
#: Off POSIX that predicate returns ``True`` as a "permissions are unreadable here"
#: sentinel rather than an observation, so ``_absence_could_be_self_inflicted`` reads the
#: absence as genuine and nothing is raised -- the documented behaviour, pinned from the
#: other side by ``test_off_posix_absence_is_inert_even_though_the_predicate_says_writable``.
#: The two tests below are the ones whose subject is the REAL predicate judging a really
#: writable ``tmp_path``, so they are meaningful only on POSIX.
#:
#: Forcing ``IS_POSIX`` True instead would be worse than skipping: the real predicate would
#: then walk its POSIX chain on a Windows runner and call
#: ``os.access(..., effective_ids=True)``, which is not implemented there. The stubbed
#: sibling ``test_a_non_root_posix_account_is_still_refused_under_the_same_stub`` keeps the
#: refusal itself covered on every platform, so no platform's answer is left untested.
_REAL_WRITABILITY_POSIX_ONLY = pytest.mark.skipif(
    os.name != "posix",
    reason=(
        "the refusal needs the real writability predicate to answer 'writable', which "
        "only POSIX can; off POSIX the guard correctly treats absence as genuine"
    ),
)
...
def managed_dir_not_user_writable(monkeypatch):
    ...
    """
    monkeypatch.setattr(ABSENCE_PRED_PATH, lambda path: False)
...
class TestTheManagedDirectoryMustNotBeUserWritable:
    ...
    def _real_predicate(self, monkeypatch):
        """Undo the module fixture: this class's subject IS the real guard."""
        monkeypatch.setattr(ABSENCE_PRED_PATH, _REAL_ABSENCE_PRED)

    @_REAL_WRITABILITY_POSIX_ONLY
    ...
    def test_absence_on_a_directory_this_account_cannot_write_is_still_inert(
        ...
        proven to refuse only the deletable case and not absence as such.
        """
        monkeypatch.setattr(ABSENCE_PRED_PATH, lambda path: False)
        _point_managed(monkeypatch, tmp_path / "dev.kirocrew.plist")
        assert governance._read_managed_policy() is None
    ...
    def test_off_posix_absence_is_inert_even_though_the_predicate_says_writable(
        self, monkeypatch, tmp_path
    ):
        """Off POSIX the predicate is a sentinel, not an observation.

        ``path_writable_by_current_user`` returns ``True`` unconditionally on a
        non-POSIX platform -- it means "permissions are unreadable here". Honouring it
        refused EVERY Windows host, which is a self-inflicted boot abort: Windows has no
        managed tier at all (``_managed_policy_path`` hands back None), so there is no
        deletion to attribute to anyone.
        """
        monkeypatch.setattr("kiro_crew.platform_compat.IS_POSIX", False)
        _point_managed(monkeypatch, tmp_path / "dev.kirocrew.plist")
        assert governance._read_managed_policy() is None

    @_POSIX_ONLY
    def test_a_sticky_group_writable_holder_reads_as_genuine_absence(self, monkeypatch, tmp_path):
        """The reported macOS abort, reproduced with a real sticky directory.

        ``/Library`` is ``drwxrwxr-t root admin`` and the default single-Mac account is
        in group ``admin``, so a "can I write here" walk answered True and EVERY
        standalone Mac that was never MDM-provisioned aborted boot. Sticky is what makes
        that answer wrong: only an entry's OWNER may unlink it, so a group member can
        add files and still cannot remove a root-owned managed document.
        """
        holder = tmp_path / "Library"
        holder.mkdir()
        holder.chmod(0o1775)  # group-writable AND sticky, as macOS ships /Library
        assert holder.stat().st_mode & stat.S_ISVTX, "precondition: sticky is set"
        _point_managed(monkeypatch, holder / "dev.kirocrew.plist")
        assert governance._read_managed_policy() is None

    @_POSIX_ONLY
    def test_the_same_holder_without_sticky_is_refused(self, monkeypatch, tmp_path):
        """The control: group-write alone IS a way in, so only sticky excuses it.

        Without this the fix would read as "group-writable directories are always fine",
        which is the opposite of what the guard is for.
        """
        holder = tmp_path / "Library"
        holder.mkdir()
        holder.chmod(0o0775)  # group-writable, NOT sticky
        _point_managed(monkeypatch, holder / "dev.kirocrew.plist")
        with pytest.raises(PlatformCompositionError, match="writable by this account"):
            governance._read_managed_policy()

    def test_a_root_owned_unwritable_holder_reads_as_genuine_absence(self, monkeypatch, tmp_path):
        """The real fleet shape, via a synthetic stat a test cannot chown into being."""
        monkeypatch.setattr("kiro_crew.platform_compat.IS_POSIX", True)
        monkeypatch.setattr(
            governance.os, "stat", lambda p, **kw: _fake_dir_stat(uid=0, mode=0o755)
        )
        _point_managed(monkeypatch, tmp_path / "dev.kirocrew.plist")
        assert governance._read_managed_policy() is None

    def test_a_root_owned_group_writable_holder_is_refused(self, monkeypatch, tmp_path):
        """Root-owned is not enough on its own: the group bit still lets a group in."""
        monkeypatch.setattr("kiro_crew.platform_compat.IS_POSIX", True)
        monkeypatch.setattr(
            governance.os, "stat", lambda p, **kw: _fake_dir_stat(uid=0, mode=0o775)
        )
        _point_managed(monkeypatch, tmp_path / "dev.kirocrew.plist")
        with pytest.raises(PlatformCompositionError, match="writable by this account"):
            governance._read_managed_policy()
    ...
    @_POSIX_ONLY
    def test_asked_as_root_the_mode_bits_still_answer(self, monkeypatch, tmp_path):
        """A root caller is answered by the DIRECTORY's mode, not by its own power.

        The earlier guard exempted uid 0 outright, so a root-run host accepted a
        deletable managed ceiling as ordinary absence. Root is no longer exempt: a
        group-writable holder is still a way in for a standard account and is still
        refused, and asking "can root write it" -- true of every directory -- is what
        the guard deliberately does not do.
        """
        monkeypatch.setattr(os, "getuid", lambda: 0)
        holder = tmp_path / "managed"
        holder.mkdir()
        holder.chmod(0o0775)  # group-writable, not sticky
        _point_managed(monkeypatch, holder / "dev.kirocrew.plist")
        with pytest.raises(PlatformCompositionError, match="writable by this account"):
    ...
    @_POSIX_ONLY
    def test_a_holder_owned_by_another_non_root_account_is_not_a_route_for_us(
        self, monkeypatch, tmp_path
    ):
        """An id-mapped filesystem must not abort boot.

        On NFS or a container userns a system directory can be owned by an arbitrary
        uid -- ``/etc`` reads as uid 65534 ``nobody`` on hosts like this one -- and an
        earlier revision of this guard treated "owned by a non-root uid" as a way in,
        which refused boot on every such host. Ownership by some OTHER account is not a
        route for THIS one; only the group/world bits or our own ownership are.
        """
        monkeypatch.setattr("kiro_crew.platform_compat.IS_POSIX", True)
        monkeypatch.setattr(os, "getuid", lambda: 1000, raising=False)
        monkeypatch.setattr(
            governance.os, "stat", lambda p, **kw: _fake_dir_stat(uid=65534, mode=0o755)
        )
        _point_managed(monkeypatch, tmp_path / "dev.kirocrew.plist")
        assert governance._read_managed_policy() is None

    def test_a_non_root_posix_account_is_still_refused_under_the_same_stub(
        self, monkeypatch, tmp_path
    ):
        """The control for the two carve-outs above: the guard still fires.

        The off-POSIX carve-out is narrow. With POSIX true and a directory this account
        owns, absence is exactly the case the guard exists for, so it must still refuse
        -- otherwise the carve-out has disabled the check rather than scoped it.
        """
        monkeypatch.setattr("kiro_crew.platform_compat.IS_POSIX", True)
        _point_managed(monkeypatch, tmp_path / "dev.kirocrew.plist")
        with pytest.raises(PlatformCompositionError, match="writable by this account"):
    ...
    def test_an_oserror_from_the_predicate_reads_as_cannot_tell_not_as_write_access(
        self, monkeypatch, tmp_path
    ):
        """A failure INSIDE the predicate is not evidence of write access.

        The guard stats the directory that would hold the document; an ``OSError`` from
        that stat means the answer is unknown. Letting it escape would turn an unreadable
        directory into a boot abort -- a permissions quirk on a path nobody configured is
        not a managed policy.
        """
        monkeypatch.setattr("kiro_crew.platform_compat.IS_POSIX", True)
        ...
        monkeypatch.setattr(governance.os, "stat", explode)
        _point_managed(monkeypatch, tmp_path / "dev.kirocrew.plist")
        assert governance._read_managed_policy() is None
