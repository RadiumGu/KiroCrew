 ...
 *
 * The prefix alone is NOT the authorization — see `isTopFrameMessage`. It is a
 ...
 * live main-process handle, not log content: `isTopFrameMessage` is what reads it.
 *
...
/**
 * Did this console message come from the dashboard's OWN top-level document?
 *
 ...
 *
 * `frame.parent` is `null` only for the top frame of the frame tree, and Chromium
 * — not the page — owns that relationship, so a document cannot lie about it. The
 * other candidate signals can be forged from inside a pane and are deliberately
 ...
 * page's to choose.
 *
 * Fails CLOSED. A runtime that delivers no `frame` (Electron < 35, where the first
 * argument is a bare event) loses the INFO-level pane journal rather than trusting
 * an unverifiable claim; the pinned `electron@^43.2.0` always supplies it.
 * Property access on a destroyed frame throws, and a throw here would take out the
 * whole console handler, so it is caught.
 */
function isTopFrameMessage(frame) {
...
function formatConsoleMessage(
  ...
  return `renderer console [${severity}]${from}: ${sanitizeLogText(message)}${where}${tail}`;
}
...
function attachFrameLoadLogging(contents, log) {
  ...
  contents.on("console-message", (...args) => {
    ...
    // identity checks out.
    const fromTopFrame = isTopFrameMessage(entry.frame);
    const allowlisted = fromTopFrame && entry.message.startsWith(PANE_LOG_PREFIX);
    if (!allowlisted && entry.severity !== "error") return;
    const limit = allowlisted ? PANE_REPEAT_LIMIT : CONSOLE_REPEAT_LIMIT;
    // Keyed by pane-chosen text, so the key set needs a ceiling of its own.
    ...
    log(formatConsoleMessage(entry, count === limit, fromTopFrame ? "" : frameOriginOf(entry.frame)));
  });
