 ...
 *
 * The prefix is a marker, not a capability: the forwarder honours it only for the
 * dashboard's own TOP-LEVEL document, because a crew pane is a cross-origin iframe
 * of the same webContents and could otherwise print these same characters to forge
 * pane records. So this module must keep being called from the dashboard document
 * itself — move a caller into an iframe or a worker and its lines stop reaching the
 * log file (they remain ordinary console output).
 */
