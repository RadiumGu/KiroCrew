          ...
          {!loading && !failed && kind === 'text' && textQ.data && (
            <>
              ...
              {/* The redactor rewrites the text silently; a reader checking a
                  ...
                  file's own bytes. Same weight as the truncation note: both say
                  "this is not the whole file". */}
              {textQ.data.redacted && (
                <p className="mb-2 text-[11px] text-muted" data-testid="drive-preview-redacted">
...
export function DriveSectionView({ account, bucket }: { account: string; bucket: string }) {
  ...
  /* A query change swaps the whole view (folder listing <-> search hits), and
     ...
     agent. So the editors close with the view they belonged to. The in-flight
     guard inside closeRename still applies -- a commit mid-flight finishes. */
  useEffect(() => {
    closeRename()
                ...
                <tbody>
                  {searchQ.data.results.map((hit) => (
                        ...
                        <div className="flex items-center justify-end gap-1">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem onSelect={() => setConfirmDelete(hit.key)} data-testid="drive-search-delete">
                    ...
                    {/* The same in-place editors the folder listing opens under
                        ...
                        both mutations refresh the results. */}
                    {renaming === hit.key && (
                        ...
                        <td colSpan={SEARCH_COLUMNS.length + 1} className="px-2.5 py-2">
                          <div className="flex flex-wrap items-center gap-2 pr-4">
                            ...
                            <Btn
                              ...
                              onClick={() => deleteMut.mutate(hit.key, { onSuccess: () => setConfirmDelete(null) })}
