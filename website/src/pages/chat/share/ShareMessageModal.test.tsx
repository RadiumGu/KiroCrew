...
describe('ShareMessageModal', () => {
  ...
  it('keeps the compose and its edits when policy withdraws sharing mid-dialog, and says why', async () => {
    ...
    // One local salvage path stays: plain-text copy of the caption (no image,
    // no third-party site), so Close never means silent loss.
  ...
  it('says so and selects the caption when the withdrawn-state text copy is refused', async () => {
    ...
    try {
      ...
      expect(screen.getByTestId('share-copy-text-unavailable')).toHaveTextContent(/clipboard unavailable/i)
