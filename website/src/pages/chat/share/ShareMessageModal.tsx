...
export default function ShareMessageModal({ onClose, messageText, prevUserText, shareEnabled }: ShareMessageModalProps) {
  ...
  // The withdrawn state's one salvage path: plain-text copy of the caption. Local
  // only (clipboard, no image, no site), so it stays available when sharing is not.
  // The clipboard can refuse (plain-HTTP hosts return false; the legacy fallback
  // can throw): the notice points the user at THIS button, so a refusal must not
  // look like success or leave the button inert — it says so, and selects the
  // caption so a manual copy is one keystroke away.
  const [textCopy, setTextCopy] = useState<'idle' | 'copied' | 'failed'>('idle')
  const captionRef = useRef<HTMLTextAreaElement>(null)
  const copyTextOnly = async () => {
    let ok = false
    try { ok = await copyToClipboard(caption) } catch { ok = false }
