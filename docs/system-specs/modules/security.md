...
- **The relocated tree is FENCED, not merely moved.** Relocating a credential store would otherwise move it out from under `security.py`'s sensitive-path gate: `_seed_pod_os_home` copies the operator's durable SSO bearer token into `<pod home>/os-home/.aws/sso/cache`, pod-minted grant pairs land beside it, and `_SENSITIVE_HOME_DIRS` entries are anchored under the real `$HOME` (plus `KIROCREW_HOME` / `KIRO_HOME`) — so an agent inside a pod could `fs_read` a verbatim copy of the operator's identity token at the pod-path spelling while the identical bytes at `~/.aws` are refused. `KIROCREW_OS_HOME` is therefore anchored as an **alternate home root** in `_resolved_root_key` / `_home_dir_targets_uncached` (and folded into the cache key, so changing it invalidates the memo), exactly as `KIROCREW_HOME` already re-anchors the crew-home leaves. **Every** `home_dirs` entry re-anchors under it, not just `.aws`: the variable relocates the whole home, so `.ssh` and every other fenced leaf move with it. The real home's anchors are unaffected — the pod root is added, never substituted.
