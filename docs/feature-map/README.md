...
| Crew Members | One durable pinned DM thread per crew member, plus a configurable webview each crew fills in | `/members` — rail row when the crew preview is on; the drawer's **Dashboard** section, expandable to full window | `pages/members/MembersPage.tsx`, `pages/members/CrewWebview.tsx` | `handlers/members.py`, `handlers/agent_panel.py` | `GET /api/members`, `POST /api/members/{slug}/thread`, `GET /api/members/{slug}/activity`, `GET /api/members/{slug}/panel`, `POST /api/agent-panel/publish`, `GET /api/agent-panel/templates` |
