...
| Crew Members | One durable pinned DM thread per crew member | `/members` — rail row when the crew preview is on | `pages/members/MembersPage.tsx`, `pages/members/chatProjection.ts`, `pages/members/EscalationCard.tsx`, `pages/members/WorkingIndicator.tsx`, `pages/members/ProcessDisclosure.tsx`, `pages/members/ChatFoldRow.tsx`, `pages/members/chatProfileRenderers.tsx` | `handlers/members.py` | `GET /api/members`, `POST /api/members/{slug}/thread`, `GET /api/members/{slug}/activity`, `GET /api/members/{slug}/conversation` |
