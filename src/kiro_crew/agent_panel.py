...
def publish(
    ...
    *,
    ...
    owner_key = crew_key(crew) if crew else ""
    ...
    with _locked(target.with_suffix(_LOCK_SUFFIX)):
        ...
        if existing is not None:
            ...
            # Empty either side is not a mismatch: a record written before ownership
            # was recorded, or a caller that named no crew, has nothing to compare.
            if owner and owner_key and owner != owner_key:
                raise PanelError(
                    "crew_slug_collision",
                    f"panel {slug!r} belongs to crew {existing.get('crew')!r}, not "
                    f"{crew_name!r}; rename one of the crews so their names do not "
                    "collide",
                )
...
def render(slug: str) -> str | None:
    ...
    """
    return render_record(read(slug))


def render_record(record: dict[str, Any] | None) -> str | None:
    """Compose an ALREADY-READ record, or ``None``.

    Split out from :func:`render` so a caller that also needs the record's fields
