...
def _apply_pod_home_remap(env: dict[str, str], *, pod_home_remap: bool) -> dict[str, str]:
    ...
    * **AWS file-based credentials deliberately do NOT follow the child into the
      ...
      What this costs, stated rather than implied: inside a pod, an agent turn
      that calls AWS resolves credentials from the ENVIRONMENT only.
      ``build_pod_env`` keeps ``AWS_*`` on purpose (its ``_TOKEN`` scrub
      excludes the ``AWS_`` prefix precisely so ``AWS_SESSION_TOKEN`` survives),
      so env-credentialed turns are unaffected. A profile that lives only in
      ``~/.aws/config`` -- including a ``credential_process`` profile -- does
      NOT resolve inside a pod any more. That is the intended posture for a
      throwaway instance whose entire purpose is to not hold machine-level
      credentials: an operator who needs AWS inside a pod exports the
      credentials into the environment, which is auditable and expires.

      An operator who sets either variable in their OWN environment still has
      it inherited (``build_pod_env`` keeps ``AWS_*``); that is their own named
