...
class AcpClient:
    ...
    async def _discard_claude_settings_seed(self) -> None:
        ...
        Async, and every step that touches the disk goes through a thread, because
        this is teardown on the event loop: the ownership hash, the durable revoke
        and the unlink are all blocking, and a heartbeat must not queue behind
        them. ``_reset_state`` keeps only the in-memory ``release``.

        **The revoke happens BEFORE the unlink, and the unlink is conditional on
        it.** ``forget`` reports whether the sidecar on disk actually stopped
        naming the path; deleting the file first and revoking after would mean a
        failed sidecar write leaves a record that outlives its file, and the next
        process would then adopt, rewrite and delete a byte-identical copy the user
        had restored. Revoking first makes the failure mode a file Crew still owns
        and a later session repairs, which is the safe direction.
        """
        if not getattr(self, "_claude_settings_authored", False):
            return
        ...
        try:
            if not await asyncio.to_thread(self._claude_settings_is_still_ours):
                logger.info(
                ...
                return
            if not await asyncio.to_thread(seed_provenance.forget, path, owner):
                # Not revoked on disk, so the file must stay: a restart would still
                # read Crew as its owner, and a later session repairs it. Only the
                # live claim is handed back, in memory, so that repair can be done
                # by a replacement client in THIS process too.
                logger.warning(
                    "could not durably revoke Crew's claim on %s; keeping the file so a "
            ...
            try:
                ...
                # The revoke already landed, so the file is now unrecorded and no
                # session may touch it -- exactly the frozen orphan this module
                # exists to remove. Re-record the bytes still on disk to put the
                # path back under Crew's ownership, so the next session recognizes
                # the orphan and re-seeds or removes it.
                logger.debug(
                ...
                if payload is not None:
                    await asyncio.to_thread(seed_provenance.record, path, payload, owner)
                    seed_provenance.release(path, owner)
        finally:
            self._claude_settings_authored = False
            self._claude_settings_written = None
    ...
    def _reset_state(self) -> None:
        ...
        # _discard_claude_settings_seed, which every caller awaits just before
        # getting here -- it is async because the ownership hash, the durable
        # revoke and the unlink are all blocking, and this method is synchronous
        # and runs on the event loop. getattr: _reset_state runs on clients built
