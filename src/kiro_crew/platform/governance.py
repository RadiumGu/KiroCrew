...
def _absence_could_be_self_inflicted(path: Path) -> bool:
    """Could a NON-ROOT account have MADE the managed document absent?

    Deliberately not "could THIS account have": that phrasing has no useful answer.
    Asked of root it is always yes, which aborted every root-run standalone install;
    asked off POSIX the underlying predicate returns a "permissions are unreadable"
    sentinel, which refused every Windows host. Both were the same mistake -- reading
    a question about the DIRECTORY off the identity of the asker. The directory's own
    owner and mode answer the real question for every caller, so this function no
    longer consults the calling account at all.

    The answer comes from the nearest EXISTING ancestor of *path* -- the directory that
    would hold the document -- and from three properties of it, in this order:

    * **Sticky** (``S_ISVTX``) -> ``False``. Sticky means only an entry's OWNER may
      unlink it, and the entry here would be root's, so group members can add files and
      still cannot remove the managed document. This case is not academic: ``/Library``
      is ``drwxrwxr-t root admin`` and the default single-Mac account is in group
      ``admin``, so asking "can I write here" answered True and aborted boot on EVERY
      standalone Mac that was never MDM-provisioned.
    * **Group- or world-writable** (and not sticky) -> ``True``. The only signal that a
      STANDARD account could have unlinked a root-owned document, and the branch that
      answers for a root caller as well.
    * **Otherwise** -> only the owner can modify it, so the answer is whether that owner
      is US. Deliberately not "is the owner root": on an id-mapped filesystem a system
      directory can be owned by an arbitrary uid (``/etc`` reads as uid 65534 ``nobody``
      on such hosts), and ownership by some OTHER non-root account is not a route for
      this one.

    Deliberately NOT the whole ancestor chain to ``/``. A writable component far above
    the managed directory does not let a standard user unlink a root-owned file inside a
    root-owned directory, and treating it as though it did is what produced the macOS
    abort above.

    Off POSIX this returns ``False``: mode bits and uids carry no usable meaning there,
    Windows has no managed tier in production (``_managed_policy_path`` hands back no
    path at all), and a test pointing the accessor at a macOS path on a Windows runner
    must not abort the loader. An ``OSError`` while statting is "cannot tell" for the
    same reason -- an unreadable directory is not evidence that anyone can write it, and
    letting it escape would turn a permissions quirk into a boot abort.
    """
    from kiro_crew.platform_compat import IS_POSIX

    if not IS_POSIX:
        return False
    # The nearest EXISTING ancestor is the directory that would hold the document. On a
    # Mac with no MDM profile ``/Library/Managed Preferences`` itself is absent, so the
    # question lands on ``/Library``.
    holder = path.parent
    try:
        ...
        logger.debug(
            "the directory that would hold %s cannot be statted; treating the absence "
            "as genuine rather than refusing boot",
            path,
            exc_info=True,
        )
        return False

    # STICKY (``t``) is decisive, and is why the old chain walk was wrong on macOS.
    # ``/Library`` is ``drwxrwxr-t root admin`` and the default single-Mac account is in
    # group ``admin``, so "can I write here" answered True and EVERY standalone Mac
    # aborted boot. Sticky means only an entry's OWNER may unlink it, and the entry here
    # would be root's -- a group member can add files and still cannot remove the managed
    # document. Group-write plus sticky is not a way in.
    if st.st_mode & stat.S_ISVTX:
        return False
    # Group or world write, without sticky, IS a way in -- and this is the branch that
    # answers for a ROOT caller too, where "can I write it" is true of every directory
    # and so says nothing. It is the only signal that a STANDARD account could have
    # unlinked a root-owned document.
    if st.st_mode & _GROUP_WORLD_WRITE:
        return True
    # Otherwise only the directory's owner can modify it, so the question reduces to
    # whether that owner is US. Deliberately NOT "is the owner root": on an id-mapped
    # filesystem (NFS, a container userns) a system directory can be owned by an
    # arbitrary uid -- ``/etc`` shows as uid 65534 ``nobody`` on hosts like that -- and
    # ownership by some OTHER non-root account is not a route for THIS one. Treating it
    # as one refused boot on every such host, which is the same self-inflicted abort in
    # a new disguise.
    getuid = getattr(os, "getuid", None)
    if getuid is None or getuid() == 0:
        # Root: already answered by the mode bits above. Asking "can root write it"
        # would be true everywhere and abort every root-run standalone install.
        return False
...
def _read_managed_policy() -> Optional[Dict[str, object]]:
    ...
    try:
        ...
        #
        # The question asked is about the DIRECTORY that would hold the document, not
        # about the calling account: "could a NON-ROOT account have removed an entry
        # here". That has a real answer for root and for a standard user alike, which
        # "can I write this" did not. On a real fleet the managed directory is
        # root-owned and not group-writable, so this is False and a standalone install
        # still reads as inert -- the compatibility contract.
        if _absence_could_be_self_inflicted(path):
            raise PlatformCompositionError(
