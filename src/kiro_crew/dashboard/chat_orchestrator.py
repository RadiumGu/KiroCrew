...
def _capture_stage_result(
    ...
    Returns the path to the result file. Fully synchronous, so callers that are
    not on the event loop (and tests) can use it directly; ``_stage_loop`` calls
    the two halves separately instead, keeping the disk work off the loop.
    """
    return _write_stage_result(slot.key, stage_num, _collect_stage_result_parts(slot))
...
async def _load_plan_budgets(slot: "_ChatSlot", tracker: OrchestrationTracker) -> bool:
    ...
    * **Dashboard Stop** has no ACP turn to cancel yet, so ``stop_turn`` answers
    ...
    try:
        ...
        logger.debug(
            "Orchestrator config load failed for slot %s; keeping the default "
            "stage and plan budgets",
            slot.key,
            exc_info=True,
        )
...
async def _stage_loop(
    ...
    try:
        ...
        if _bootstrapping and not await _load_plan_budgets(slot, tracker):
            return
        for stage_idx in range(start_idx, total):
            ...
            # the loop (it reads live slot state), the mkdir + write go to a
            # worker. It used to be one synchronous call on the loop.
            try:
