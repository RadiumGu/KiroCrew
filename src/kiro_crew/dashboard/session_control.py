...
def _escalation_home(
    state: "DashboardState", caller_slot: "_ChatSlot", caller_key: str
) -> tuple["_ChatSlot", str]:
...
async def escalate_to_user(
    ...
    * **Escalation is not approval.** This path carries a decision the member
    ...
    home, slug = _escalation_home(state, caller_slot, caller_key)
    ...
    if slug:
        ...
        try:
            ...
            raise SessionControlError(
                ...
                "as a plain message",
                code="escalation_index_unavailable",
                status=500,
            ) from exc
    ...
    live_caller = state.get_slot(caller_key)
    if state.get_slot(home.key) is not home or (
        home is not caller_slot
        and getattr(home, "workspace", "default") != getattr(live_caller, "workspace", "default")
    ):
        await asyncio.to_thread(_retract)
        raise SessionControlError(
            "the thread this escalation was addressed to closed or changed workspace "
    ...
    try:
        ...
        # no-deadline record would keep the badge lit forever.
        await asyncio.to_thread(_retract)
        raise
